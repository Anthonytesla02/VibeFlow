...
import { Home, Library, Disc, Search } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, currentPage, onNavigate }) => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-[#0f0f11] overflow-hidden">
      ...
      {/* Mini Player (Sticky if not on player page) */}
      {currentPage !== 'player' && currentSong && (
        <div 
          onClick={() => onNavigate('player')}
          className="fixed bottom-16 left-0 right-0 h-16 bg-[#18181b] border-t border-white/10 flex items-center px-4 cursor-pointer z-40"
        >
          <img 
      ...
      {/* Bottom Navigation */}
      <nav className="h-16 bg-[#09090b] border-t border-white/5 flex items-center justify-around z-50 fixed bottom-0 w-full">
        {navItems.map((item) => {
           ...
           return (
             <button
                ...
                className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive ? 'text-green-400' : 'text-gray-500'}`}
             >
